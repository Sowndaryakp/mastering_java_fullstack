...
const ExamResults: React.FC = () => {
  ...
  const loadData = async () => {
    try {
      ...
      const feedbackState: { [key: number]: string } = {};
      resultsResponse.data.forEach((result: ExamResult) => {
  ...
  const handleFeedbackChange = (resultId: number, value: string) => {
    setFeedback((prev) => ({
      ...prev,
      [resultId]: value,
    }));
  ...
  const handleFeedbackSubmit = async (resultId: number) => {
    try {
